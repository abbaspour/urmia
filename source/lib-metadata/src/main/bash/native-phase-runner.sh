...
[ -e "$opt_inpp" ] || {
    echo >>${verbose} "input does not exist: $opt_inpp. creating one";
    mkfifo ${opt_inpp}
}
...
if [ ${opt_xargs} -ne 0 ]; then
    [ -z "$verbose" ] || echo "running: /usr/bin/xargs -L1 ${opt_cmd} to output: ${opt_outp}" >>${verbose}
    /usr/bin/xargs -t -L1 ${opt_cmd} >>${opt_outp} 2>>${verbose}
else
    [ -z "$verbose" ] || echo "running: ${opt_cmd}" >>${verbose}
    ${opt_cmd}
fi

declare -r -i exitCode=$?
[ -z "$verbose" ] || echo "exit code: ${exitCode}" >>${verbose}
